...
st.set_page_config(page_title="Backtest Estratégico", layout="wide")
...
st.title("📈 Backtest Estratégico com CSV")

data_file = st.file_uploader("📁 Envie o seu arquivo CSV com dados históricos:", type=["csv"])
...
if data_file:
    ...
    st.subheader("⚙️ Seleção de Setup")
    setup = st.selectbox("Escolha o setup para análise:", ["INVERT 50", "9-50-20", "PC"])
    ...
    if st.button("🚀 Executar Backtest"):
        ...
        if "setup" in df.columns:
            ...
            col1.metric("Total de Trades", f"{total_trades}", f"Win: {wins} | Loss: {losses} | BE: {breakeven}")
...
{saved['texto']}")
    ...
    st.warning("⚠️ Envie um arquivo CSV para começar.")
